...
template <int CoarsenFactor>//CoarsenFactor粗化向量4或2或1
__global__ void csrspmm_non_transpose_seqreduce_rowbalance_kernel(
    ...
    /*关于indices和indptr的总结：
      ...
      indptr: 0 3 6 11 ……
*/
  int row = blockIdx.x * blockDim.x + threadIdx.x;//1D坐标
  int stride = blockDim.x * gridDim.x;//线程增量

  // get the dense column offset 列偏移量
  int col_offset = blockIdx.y * CoarsenFactor;
  int ldB = K;
  int ldC = M;
  const float *B_panels[CoarsenFactor];
...
#pragma unroll
  ...
  for (; row < M; row += stride) {
    ...
    int start = csr_indptr[row];//当前行开始元素序号
    int end = csr_indptr[row + 1];//当前行结束元素序号
    int k;
    float v;

    for (int p = start; p < end; p++) {
      k = csr_indices[p];//当前行第几列
...
#pragma unroll
  ...
  if (eid < nnz) {
    ...
    //此函数用于查找csr_indptr中大于eid的第一个元素（n^th），并将n-1输出到row
    int step = csr_indptr[row + 1] - eid;//当前eid到下一个的距离
    int k;
    float v;
